...
def trace_trajectory_Astar(d0, goal_pos, tf, freq, gri_open=True): #达不到柔顺控制的要求（姿态方面达不到，位置方面可以达到）
    ...
    if Points_recover is not None:
        X, Y, Z = plan.path_smoothing(Path_points=Points_recover, t_final=tf, freq=freq)  ##轨迹使用二次B样条曲线进行平滑处理
    else:
...
# 第一段轨迹
print("-------------------------------------------------------------------------------------------------", env.door_joint_pos)
print("-------------------------------------------------------------------------------------------------", env._handle_xpos)
...
# 检验t
t1 = env.sim.data.time
obs_3, _, force2 = trace_trajectory_Astar(d_0, goal_pos, tf=t_f, freq=20, gri_open=False)
print('delta_t=', env.sim.data.time - t1)
print('handle_pos_fin=', env._handle_xpos)
print('slide_handle_pos_fin=', env._slide_handle_xpos)
print(env.cabinet_handle_qpos, "############################################################################")
