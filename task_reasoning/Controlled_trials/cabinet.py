...
class Cabinet(SingleArmEnv):
    ...
    def _load_model(self):
        ...
        #     lock=self.use_latch,
        # )
        self.cabinet = CabinetObject(name="CabinetObject")

        # Create placement initializer
        if self.placement_initializer is not None:
            self.placement_initializer.reset()
            self.placement_initializer.add_objects(self.cabinet)
        else:
    ...
    def _setup_references(self):
        ...
        self.object_body_ids['cabinet'] = self.sim.model.body_name2id(self.cabinet.cabinet_base)
        self.object_body_ids['cabinet_drawer_link'] = self.sim.model.body_name2id(
            self.cabinet.cabinet_drawer_link)
        self.cabinet_object_site_id = self.sim.model.site_name2id(self.cabinet.important_sites['slide_handle'])
        self.cabinet_qpos_addrs = self.sim.model.get_joint_qpos_addr(self.cabinet.joints[0])
    ...
    def _setup_observables(self):
        ...
        if self.use_object_obs:
            ...
            def door_pos(obs_cache):
                return np.array(self.sim.data.xpos[self.object_body_ids["cabinet"]])
            ...
            def door_to_eef_pos(obs_cache):
                return (
                    obs_cache["cabinet_pos"] - obs_cache[f"{pf}eef_pos"]
                    if "cabinet_pos" in obs_cache and f"{pf}eef_pos" in obs_cache
                    else np.zeros(3)
                )
            ...
            def handle_to_eef_pos(obs_cache):
                return (
                    obs_cache["slide_handle_xpos"] - obs_cache[f"{pf}eef_pos"]
                    if "slide_handle_xpos" in obs_cache and f"{pf}eef_pos" in obs_cache
                    else np.zeros(3)
                )

            @sensor(modality=modality)
            def hinge_qpos(obs_cache):
                return np.array([self.sim.data.qpos[self.cabinet_qpos_addrs]])
    ...
    def _reset_internal(self):
        ...
        if not self.deterministic_reset:
            ...
            cabinet_pos, cabinet_quat, _ = object_placements[self.cabinet.name]
            cabinet_body_id = self.sim.model.body_name2id(self.cabinet.root_body)
    ...
    def _check_success(self):
        ...
        """
        slide_qpos = self.sim.data.qpos[self.cabinet_qpos_addrs]
        return slide_qpos > 0.08
    ...
    def visualize(self, vis_settings):
        ...
        if vis_settings["grippers"]:
            self._visualize_gripper_to_target(
                gripper=self.robots[0].gripper, target=self.cabinet.important_sites["slide_handle"], target_type="site"
            )

    @property
    def _handle_xpos(self):
        """
        Grabs the position of the door handle.
