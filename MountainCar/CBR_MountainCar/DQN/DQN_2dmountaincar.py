...
def case_generation(state, action):
    # 注意：不要忘记在案例库生成函数定义之前定义一个案例库
    global case_base
    case_base = np.vstack((case_base, np.append(state, action)))
    return case_base
        ...
        if random.random() > self.epsilon:
            with torch.no_grad():
                ...
                action = q_values.max(1)[
                    1
                ].item()  # choose action corresponding to the maximum q value
        else:
...
def train(cfg, env, agent):
    ...
    for i_ep in range(cfg["train_eps"]):
        ...
        for _ in range(cfg["ep_max_steps"]):
            ...
            if (
                i_ep >= 150
            ):  # 为了保证策略的良好性，这里训练100个episode策略再记录入案例库
                case_generation(state, action)
...
def get_args():
    ...
    parser.add_argument(
        "--algo_name", default="DQN", type=str, help="name of algorithm"
    )
    parser.add_argument(
        "--env_name", default="MountainCar-v0", type=str, help="name of environment"
    )
    parser.add_argument(
        "--train_eps", default=350, type=int, help="episodes of training"
    )
    ...
    parser.add_argument(
        "--epsilon_start", default=0.95, type=float, help="initial value of epsilon"
    )
    parser.add_argument(
        "--epsilon_end", default=0.01, type=float, help="final value of epsilon"
    )
    parser.add_argument(
        "--epsilon_decay",
        default=500,
    ...
    parser.add_argument(
        "--memory_capacity", default=100000, type=int, help="memory capacity"
    )
...
# 将案例库保存为npy文件
np.save("case_base.npy", case_base)
